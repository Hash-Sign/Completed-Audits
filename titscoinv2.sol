...
/**
   ...
   */
   ...
   contract TitscoinV2 is Context, IERC20, Ownable {
      ...
      address[] private _blackListedBots;
       ...
       string private _name = "TitscoinV2";
       ...
       uint256 private numTokensSellToAddToLiquidity = 500000 * 10**1 * 10**9;
       ...
       constructor (address payable charityWalletAddress, address payable marketingWalletAddress) public {
           ...
           _whaleTaxLimit = _tTotal.div(1000); // sets additional tax for whale transactions
       ...
       function _getTValues(uint256 tAmount, uint256 taxFee, uint256 liquidityFee) private view returns (uint256, uint256, uint256) {
           uint256 tFee = ((tAmount.mul(taxFee)).div(100));
           uint256 whaleTax = calculateWhaleTax(tAmount);
           uint256 tLiquidity = ((tAmount.mul(liquidityFee.add(whaleTax))).div(100));
       ...
       function calculateWhaleTax(uint256 _amount) private view returns (uint256){
         uint whaleTax = 0;
         if(!_takeWhaleTax) {
           return 0;
         }
         if (_amount > _whaleTaxLimit) {
           whaleTax = _amount.mul(1000).div(_tTotal);
         }
         return whaleTax.mul(_whaleFee);
       }
       ...
       function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
           ...
           uint256 halfOfLiquify = contractTokenBalance.div(7);
           uint256 otherHalfOfLiquify = contractTokenBalance.div(7);
           ...
           sendBNBToCharity(portionForFees.div(5).mul(3));
           sendBNBToMarketing(portionForFees.div(5).mul(2));
